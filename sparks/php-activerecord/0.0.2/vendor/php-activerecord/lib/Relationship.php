...
abstract class AbstractRelationship implements InterfaceRelationship
{
	...
	protected function query_and_attach_related_models_eagerly(Table $table, $models, $attributes, $includes=array(), $query_keys=array(), $model_values_keys=array())
	{
		...
		if (!empty($options['through'])) {
			...
			$include_connection = array('select' => $this->attribute_name.'.*, '.$through_table->table.'.'.$query_key);
		...
		foreach ($models as $model)	//maintenance_plan model
		{
			$matches = 0;
			$key_to_match = $model->$model_values_key;

			foreach ($related_models as $related)	//service model
			{
				// service.id == maintenance_plan.id
				if ($related->$query_key == $key_to_match)	//should be service_to_maintenance[ service.id ] == maintenance_plan.id
				{
